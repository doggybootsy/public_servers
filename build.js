...
(async () => {
  if (!process.argv.includes("--no-build")) {
    await esbuild.build({
      ...
      outfile: "dist/publicServers.plugin.js",
      platform: "node",
      bundle: true,
      format: "cjs",
      minifySyntax: true,
    ...
    const pluginData = readFileSync("dist/publicServers.plugin.js", { encoding: "utf-8" });
    
    const pluginCode = `/**\n${Object.entries(meta).map(([ k, v ]) => ` * @${k} ${v}`).join("\n")}\n */\n${pluginData}`;
    writeFileSync("dist/publicServers.plugin.js", pluginCode, { encoding: "utf-8" });
    console.log("Built");
    ...
    if (process.argv.includes("--bd")) {
      ...
      writeFileSync(join(path, "plugins", "publicServers.plugin.js"), pluginCode, { encoding: "utf-8" });
